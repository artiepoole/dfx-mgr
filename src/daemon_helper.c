...
static int fpga_state(void)
{
	...
	if (system("cat /sys/class/fpga_manager/fpga0/state >> /etc/dfx-mgrd/state.txt")) {
	...
	if (fptr) {
		...
		fclose(fptr);
		if (system("rm /etc/dfx-mgrd/state.txt")) {
			DFX_ERR("Failed system() API");
		}
...
static int user_load_overlay(char *ov, char *region)
{
	...
	snprintf(command, sizeof(command), "cat %s/path >> /etc/dfx-mgrd/state.txt", ov_dir);
	if (system(command)) {
		DFX_ERR("Failed system() API");
	}
